...
//source https://gist.github.com/yasirkula/06edc780beaa4d8705b3564d60886fa6
namespace UIYellowPicker
{
    public class SceneViewUIObjectPickerContextWindow : EditorWindow
    {
        ...
        private void ShowContextWindow(List<Entry> results)
        {
            ...
            Rect rect = new Rect(GUIUtility.GUIToScreenPoint(Event.current.mousePosition) - new Vector2(0f, position.height),
                                 position.size); // Show dropdown above the cursor instead of below the cursor
        ...
        private static Rect GetScreenFittedRect(Rect originalRect, EditorWindow editorWindow)
        {
            ...
            else
            {
                ...
                hostViewContainerWindowGetter ??= typeof(EditorWindow).Assembly.GetType("UnityEditor.HostView")
                                                                      .GetProperty("window", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);

                return (Rect)screenFittedRectGetter.Invoke(null,
                                                           new object[]
                                                           {
                                                               originalRect, originalRect.center, true,
                                                               hostViewContainerWindowGetter.GetValue(editorWindowHostViewGetter.GetValue(editorWindow), null)
                                                           });
            }
        }
    }
