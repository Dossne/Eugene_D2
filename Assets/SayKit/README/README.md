...
Note: be careful with the compression method on Android. We recommend using the default value.
...
Add `APP_NAME_CHINA_IOS` and `APP_NAME_IOS` if you want to localize application name for China store.
...
### Install Gradle 8.8 and JDK 17:

Please, check the guide on our help page:

 <a href='https://launcher.saygames.io/help/127FLIEgS6vLLnSpM6zvfynDLpTjs57OTyxSaNbWW21U'> - [EN] Gradle 8.8 and JDK 17 guide.</a> 


### Fixes for Unity 2020.3+
...
// Track event if you are ready to show the ad.
void trackInterstitialOffer(string place, string extra = "")
void trackRewardedOffer(string place, string extra = "")

// Events for levels
void trackLevelStarted(int level);
void trackLevelStarted(string tag, int level);
void trackLevelStarted(string tag, int level, int score, string extra1);
void trackLevelStarted(string tag, int level, int score, string extra1, int number, string extra2);

void trackLevelCompleted(int level, int score);
void trackLevelCompleted(string tag, int level, int score);
void trackLevelCompleted(string tag, int level, int score, string extra1);
void trackLevelCompleted(string tag, int level, int score, string extra1, int number, string extra2);

void trackLevelFailed(int level, int score);
void trackLevelFailed(string tag, int level, int score);
void trackLevelFailed(string tag, int level, int score, string extra1);
void trackLevelFailed(string tag, int level, int score, string extra1, int number, string extra2);

// Events for stages
void trackLevelStageStarted(int number);
void trackLevelStageStarted(string tag, int number);
void trackLevelStageStarted(string tag, int number, string extra1, string extra2);

void trackLevelStageCompleted(int number, int score);
void trackLevelStageCompleted(string tag, int number, int score);
void trackLevelStageCompleted(string tag, int number, int score, string extra1, string extra2);

void trackLevelStageFailed(int number, int score);
void trackLevelStageFailed(string tag, int number, int score);
void trackLevelStageFailed(string tag, int number, int score, string extra1, string extra2);

// Events for extra levels
void trackLevelExtraStarted();
void trackLevelExtraStarted(string tag);
void trackLevelExtraStarted(string tag, string extra1);
void trackLevelExtraStarted(string tag, int number, string extra1, string extra2);

void trackLevelExtraCompleted(int score);
void trackLevelExtraCompleted(string tag, int score);
void trackLevelExtraCompleted(string tag, int score, string extra1);
void trackLevelExtraCompleted(string tag, int score, int number, string extra1, string extra2);

void trackLevelExtraFailed(int score);
void trackLevelExtraFailed(string tag, int score);
void trackLevelExtraFailed(string tag, int score, string extra1);
void trackLevelExtraFailed(string tag, int score, int number, string extra1, string extra2);

// Events for chunks
void trackChunkStarted(string name, int sequenceNumber);
void trackChunkStarted(string name, int sequenceNumber, Dictionary<string, object> customData);
void trackChunkCompleted();
void trackChunkCompleted(Dictionary<string, object> customData);
void trackChunkFailed();
void trackChunkFailed(Dictionary<string, object> customData);


// Events for items unlock/receive
void trackItem(string item);
void trackItem(string item, int ownedItems, SourceType sourceId, Dictionary<string, object> customData);

// Event for opened screen
void trackItemLoss(string item, int ownedItems, SourceType sourceId, Dictionary<string, object> customData);

//Event for clicked element on screen
void trackClick(string screen, string element);

// Event for track changing of screen
...
// Events with a tag
void trackTagEvent(string eventName, string tag, int eventParam1, int eventParam2, string eventParam3, string eventParam4);
void trackTagEvent(string eventName, string tag, int eventParam1, int eventParam2, int eventParam3, string eventParam4, string eventParam5);
void trackTagEvent(string eventName, string tag, int eventParam1, int eventParam2, int eventParam3, int eventParam4, string eventParam5, string eventParam6);

// Third-party events (Facebook/Firebase)
void logFBAppEvent(string logEvent, float? valueToSum = default(float?), Dictionary<string, object> parameters = null)
void logFirebaseEvent(string eventName, string extraParam = "")
void logFirebaseEvent(string eventName, string paramName, object paramValue)
void logFirebaseEvent(string eventName, float? valueToSum = default(float?), Dictionary<string, object> parameters = null)
void setCrashlyticsParam(string paramName, string paramValue)
void logCrashlyticsException(Exception exception)
...
void trackPurchase(Product purchasedProduct)
void SayKit.enablePremium()
bool SayKit.isPremium

// Event for track application loading.
void trackAplicationLoaded()

// Rate App. Returns true if popup was shown. Popup is shown once during lifetime of the app. 
bool SayKit.showRateAppPopup()
// Custom Rate App. Returns true if popup was shown. Notify SayKit that your custom pop up was shown.
bool showCustomRateAppPopup(int rate = 0)

// Tutorial steps
void trackTutorialCompleted();
void trackTutorialStep(string tutorialName, string stepName);
...
// Manual notification token request. You will get a token in the notificationTokenReceived callback.
void RequestNotificationToken();
string GetNotificationToken();

// Open browser with support page. Returns request GUID id.
string OpenSupportPage();

// Returns the privacy policy link.
string GetPrivacyPolicyLink();

// Localizations
string getLocalizedString(string key, string val1 = null, string val2 = null, string val3 = null, string val4 = null, string val5 = null);
// Returns true if uses default(english) localization.
(string, bool) getLocalizedTuple(string key, string val1 = null, string val2 = null, string val3 = null, string val4 = null, string val5 = null);
bool hasLocalizedMessage(string key)
// Changes the system language and saves it in future sessions.
void overrideSystemLanguage(SayKitLanguage language);
SayKitLanguage getCurrentLanguage();

// FPS
void startFPSTag(string tagName);
void endFPSTag(string tagName);
void startTimeFPSTag(string tagName, int seconds);

// [Android] Opens a link to Google Play's Subscription Center.
void OpenGooglePlaySubscriptionCenter(string productId)
```


## GDPR

SayKit will automatically determine if user is the subject of the GDPR and will show popup to obtain consent.
...
If you want to change the default notification icon on Android, you need to add notification_icon image manually:
<a>https://saygames.notion.site/saygames/How-to-add-Android-push-notification-icon-ad04dca85bac4685a8629f81f822c6ad</a>
